...
import {
  ...
  LlamaDialogModule,
  LlamaDialogRef,
} from '@shared/ui/llama-dialog';
import {
  LLAMA_DRAWER_DATA,
  LlamaDrawer,
  LlamaDrawerModule,
  LlamaDrawerRef,
} from '@shared/ui/llama-drawer';

@Component({
  selector: 'llama-drawer-example',
  standalone: true,
  template: `
    <llama-drawer-header>
      <h1>Title</h1>
    ...
    <llama-drawer-actions>
      <button llama-button>Cancel</button>
      <button llama-button>Primary</button>
    </llama-drawer-actions>
  `,
  imports: [LlamaButtonModule, LlamaDrawerModule],
})
export class LlamaDrawerExample {
  constructor(
    readonly dialogRef: LlamaDrawerRef,
    @Inject(LLAMA_DRAWER_DATA) public data: any
  ) {
    console.log(this.data, dialogRef);
  }
}

@Component({
  selector: 'llama-modal',
  standalone: true,
  template: `
    <llama-dialog-content>
      <p>Test</p>
    </llama-dialog-content>
    <llama-dialog-actions [align]="'center'">
      <button llama-button [llama-dialog-close]="{ data: '' }">Ok</button>
    </llama-dialog-actions>
  `,
  imports: [LlamaDialogModule, LlamaButtonModule],
})
export class LlamaModal {
  constructor(
    readonly dialogRef: LlamaDialogRef,
    @Inject(LLAMA_DIALOG_DATA) public data: any
  ) {
    console.log(this.data, dialogRef);
  }
}
...
@Component({
  selector: 'llama-dashboard-ecommerce',
  standalone: true,
  imports: [],
  template: `
    <p>dashboard-ecommerce works!</p>

    <ng-template #template>Buenas from template</ng-template>
...
export default class DashboardEcommerceComponent {
  private _llamaDialog = inject(LlamaDialog);
  private _llamaDrawer = inject(LlamaDrawer);

  template = viewChild.required<TemplateRef<any>>('template');

  open() {
    const dialogRef = this._llamaDialog.open<
      LlamaModal,
      { test: string },
      { data: string }
    >(LlamaModal, {
  ...
  openDrawer() {
    const dialogRef = this._llamaDrawer.open<
      LlamaDrawerExample,
      { test: string },
      { data: string }
    >(LlamaDrawerExample, {
